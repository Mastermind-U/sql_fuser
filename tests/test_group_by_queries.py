...
def test_group_by_single_column() -> None:
    ...
    assert query == (
        ...
        'FROM "users" AS "a" GROUP BY "a"."department"'
    )
    assert params == ()
...
def test_group_by_cube_with_where_clause() -> None:
    ...
    assert query == (
        ...
        'WHERE "a"."year" = ? '
        "GROUP BY "
        'CUBE ("a"."dimension1", "a"."dimension2")'
    )
    assert params == (2024,)
...
def test_group_by_grouping_sets_with_empty_set() -> None:
    ...
    assert query == (
        ...
        'FROM "data" AS "a" '
        "GROUP BY "
        'GROUPING SETS (("a"."col1", "a"."col2"), ("a"."col1"), ())'
    )
    assert params == ()
...
def test_group_by_grouping_sets_with_where_clause() -> None:
    ...
    assert query == (
        ...
        'WHERE "a"."status" = ? '
        "GROUP BY "
        'GROUPING SETS (("a"."year", "a"."month"), ("a"."year"), ())'
    )
...
def test_group_by_grouping_sets_with_having() -> None:
    ...
    assert query == (
        ...
        'FROM "reports" AS "a" '
        "GROUP BY "
        'GROUPING SETS (("a"."region", "a"."zone"), ("a"."region"), ()) '
        'HAVING "a"."revenue" = ?'
    )
    assert params == (">10000",)
